<!-- Archived superseded draft; not used by either compiler. -->
# Chapter 0 Companion Guide: From Nothing to Ready

## The Complete Beginner's Mathematical & Computational Reference Manual
...
Whether you want to read this side-by-side with `chapter0.md` or convert it to HTML using tools like Pandoc, MathJax, or KaTeX, all mathematical notation is strictly formatted using standard LaTeX delimiters (`$inline$` and `$$display$$`).
...
### The Core Pedagogical Contract

1. **One Central Idea Per Day:** Systematic, day-by-day progression without skipping foundational steps.
...
| --- | --- | --- | --- |
| Vector Dot Product | $\sum_{i=1}^{p} a_i b_i$ | `sum(a[i] * b[i] for i in range(p))` | `a @ b`<br> |
| Sample Mean | $\bar{x} = \frac{1}{n} \sum x_i$ | `sum(x) / len(x)` | `x.mean()`<br> |
| Mean Centering | $x_i - \bar{x}$ | `[val - mean_x for val in x]` | `x - x.mean()`<br> |
| Sum of Squared Errors | $\sum (y_i - \hat{y}_i)^2$ | `sum((y[i] - y_hat[i])**2 for i in range(n))` | `np.sum((y - y_hat)**2)`<br> |
| Population Variance | $\frac{1}{n}\sum (x_i - \bar{x})^2$ | `sum((x[i] - mean_x)**2 for i in range(n)) / n` | `x.var()`<br> |
| Covariance | $\frac{1}{n}\sum (x_i - \bar{x})(y_i - \bar{y})$ | `sum((x[i]-mx)*(y[i]-my) for i in range(n)) / n` | `np.cov(x, y, ddof=0)[0,1]`<br> |

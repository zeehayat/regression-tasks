...
If that sounds ambitious, good news: nobody is expected to arrive knowing the vocabulary. Treat this chapter as a workshop, not an exam. Type the small examples, laugh at the inevitable error messages, and repeat an exit check whenever it still feels foggy.
...
<div style="margin: 14px 0 20px 0;">
    <button type="button" onclick="openDay0EVideoModal()" style="background: linear-gradient(135deg, #ec4899, #db2777); color: #ffffff; border: none; padding: 10px 22px; border-radius: 12px; font-size: 14px; font-weight: 800; cursor: pointer; box-shadow: 0 6px 20px rgba(236, 72, 153, 0.4); display: inline-flex; align-items: center; gap: 8px; transition: transform 0.15s ease;" onmouseover="this.style.transform='scale(1.02)'" onmouseout="this.style.transform='scale(1)'">
        <span>🎬 Watch Day 0E Masterclass Video (Female Voice & Interactive Exercises)</span>
    </button>
</div>
...
## 0E.5a Symbolic bridge: differentiate squared error by hand

Numerical slopes tell us *what* a derivative is. Later chapters also manipulate symbols to obtain a reusable rule. Start with one observation, treating $x$, $y$, and $c$ as fixed numbers while $m$ is the adjustable slope:

$$
E(m)=(y-mx-c)^2.
$$

First expand the square. Using $(a-b)^2=a^2-2ab+b^2$ with $a=y-c$ and $b=mx$:

$$
E(m)=(y-c)^2-2(y-c)mx+m^2x^2.
$$

Now differentiate term by term with respect to $m$:

$$
\frac{dE}{dm}=0-2(y-c)x+2mx^2.
$$

Factor the result to recover the compact chain-rule form:

$$
\frac{dE}{dm}=-2x(y-mx-c).
$$

For $x=2$, $y=10$, $c=1$, and $m=1$, the expanded polynomial is $E(m)=81-36m+4m^2$, so its derivative is $-36+8m=-28$ at $m=1$.

```python
def symbolic_gradient_m(x, y, m, c):
    return -2 * x * (y - m * x - c)

x, y, m, c = 2.0, 10.0, 1.0, 1.0
by_algebra = symbolic_gradient_m(x, y, m, c)
by_finite_difference = partial_wrt_m(error, m, c, h=1e-6)

print(by_algebra, by_finite_difference)
assert by_algebra == -28.0
assert abs(by_algebra - by_finite_difference) < 1e-4
```

The letters are placeholders, not mysterious objects. Expanding, differentiating term by term, refactoring, and checking numerically is the same workflow used for the matrix gradient in Chapter 1 and the regularisation derivatives in Chapter 3.

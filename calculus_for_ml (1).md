...
**Suggested pace:** Parts 1–4 in one sitting, Parts 5–6 in a second, Parts 7–8 in a third, Part 9
(the capstone) in a fourth, once 1–8's exit checks all pass. Do the exercises between sittings, not
after all of them.
...
# Part 9 — Capstone: the whole toolchain, on the real dataset

Every part so far worked on a toy: one or two data points, one or two parameters, chosen so the
arithmetic stays checkable by hand. That was deliberate — it is how you learn *what* a tool does.
This part changes exactly one thing: scale. Every function below is the one you already built.
None of it is new machinery.

## 9.1 The real design matrix

Three features, forty projects, the same columns from Chapter 0's own examples:
...
cable_km      = np.random.uniform(5, 60, n)
capacity_kw   = np.random.uniform(50, 600, n)
terrain_index = np.random.randint(1, 6, n).astype(float)

X_raw = np.column_stack([cable_km, capacity_kw, terrain_index])
```

The true relationship is deliberately nonlinear and deliberately noisy — the two things every
toy example in Parts 1–8 avoided on purpose:

```python
beta_true = np.array([0.9, 0.05, 3.0])
intercept_true = -32.0

def softplus(z): return np.logaddexp(0.0, z)   # Part 3.7
def sigmoid(z):  return 1.0 / (1.0 + np.exp(-z))

z_true = X_raw @ beta_true + intercept_true
cost = softplus(z_true) + np.random.normal(0, 0.6, n)
```

Cost cannot be negative *by the model* — softplus guarantees that — but measurement noise can
still push an observed value slightly below zero for a project whose true cost is near zero. That
is not a contradiction; it is the difference between what the model predicts and what you
actually measured, and real datasets make you hold both ideas at once.

Standardise, exactly as section 5.1 said you must, because the three features here live on
wildly different scales (kilometres, kilowatts, a 1–5 index):
...
$$\text{predict}(\beta, b) = \text{softplus}(X\beta + b)$$

This is Part 3.7's function, wrapped around Part 6's linear combination. Nothing here is a new
idea — it is two old ideas composed, which is exactly what "the chain rule" is a name for.

## 9.3 The gradient, by hand, using section 3.7's result directly

$$L(\beta, b) = \frac{1}{n}\sum \big(\text{softplus}(X\beta+b) - y\big)^2, \qquad z = X\beta + b$$

Chain rule, three layers deep, exactly as section 2.4 described the procedure:

$$\frac{\partial L}{\partial z} = \frac{2}{n}\big(\text{softplus}(z) - y\big)\cdot\sigma(z)$$

The $\sigma(z)$ factor is not an approximation or a new rule — it is section 3.7's identity,
$\frac{d}{dz}\text{softplus}(z) = \sigma(z)$, substituted in directly. From there:

$$\nabla_\beta L = X^\top \frac{\partial L}{\partial z}, \qquad \frac{\partial L}{\partial b} = \sum \frac{\partial L}{\partial z}$$

```python
def predict(beta, b, X):
    return softplus(X @ beta + b)

def loss_fn(beta, b, X, y):
    return np.mean((predict(beta, b, X) - y) ** 2)

def grad_analytic(beta, b, X, y):
    n = X.shape[0]
    z = X @ beta + b
    dL_dz = (2.0 / n) * (softplus(z) - y) * sigmoid(z)   # <- the chain rule step
    return X.T @ dL_dz, np.sum(dL_dz)
```

## 9.4 Gradient check, reused verbatim from Part 8.2

Not a new function — the exact `gradient_check` from section 8.2, called on the flattened
parameter vector $[\beta_1, \beta_2, \beta_3, b]$:

```python
p0 = np.array([0.1, 0.1, 0.1, 0.0])
rel_err, analytic, numeric = gradient_check(flat_loss, flat_grad, p0)
...
```text
analytic: [-11.00410165  -2.432957    -5.88823038 -23.84498076]
numeric : [-11.00410162  -2.43295705  -5.88823042 -23.84498083]
relative error: 1.7534952312427802e-09
```

$10^{-9}$ — well inside section 8.2's "below $10^{-7}$ is fine" bar. The three-layer chain rule
is correct.

## 9.5 Break it deliberately: drop the sigmoid factor

Same mistake as section 2.6, at this larger scale:

```python
def grad_broken(beta, b, X, y):
    n = X.shape[0]
    dL_dz = (2.0 / n) * (softplus(X @ beta + b) - y)   # missing * sigmoid(z)
    return X.T @ dL_dz, np.sum(dL_dz)
```

```text
broken gradient (dropped sigmoid factor), relative error: 0.3133500881887404
```

$0.31$ against $10^{-9}$. No traceback, no exception — Python ran both versions without
complaint. Section 8.2's warning was not decorative.

## 9.6 The Hessian is not a bowl everywhere, and that is the point

Part 4 and Part 6's Hessians were always positive semidefinite because squared error on a
*linear* model is always convex. Squared error on a *nonlinear* model is not guaranteed to be:

```python
H_start = hessian_numeric(flat_loss, p0)
print(np.linalg.eigvalsh(H_start))
```

```text
[-17.26735833 -11.89733618  -8.68335182  -1.54686731]
```

All four eigenvalues negative, far from the minimum. This is section 4.3's "wiggly function with
several dips," measured rather than described. It is also the honest, unglamorous reason deep
learning theory is harder than linear regression theory: the bowl guarantee from Part 4 simply
does not survive composing a nonlinear activation with squared error.

Train anyway — gradient descent does not require global convexity, only that you not step too
far at any one point:

```python
def run_gd(p_start, eta, steps):
    p = p_start.copy()
    history = [flat_loss(p)]
    for _ in range(steps):
        p = p - eta * flat_grad(p)
        history.append(flat_loss(p))
    return p, history

p_fit, history = run_gd(p0, eta=0.002, steps=4000)
print(p_fit, history[-1])
```

```text
[13.47269139  7.94178019  4.03673802 23.21092621] 0.286273697854473
```

Now recompute the Hessian **at this point**, not at the start:

```python
print(np.linalg.eigvalsh(hessian_numeric(flat_loss, p_fit)))
```

```text
[0.51027362 1.53538736 1.80979961 2.5842461]
```

All positive. Near the minimum this particular surface *does* look like a bowl — Part 7.4's
theory was never wrong, it was just answering a question ("how large can $\eta$ safely be
**here**") that only has a stable answer once you are close enough to a minimum for the local
quadratic approximation to hold. Section 8.5's Taylor logic is exactly what "close enough" means.

## 9.7 The stability bound, tested where it actually applies

$$\eta < \frac{2}{L}, \qquad L = \lambda_{\max}(H) = 2.584 \;\Rightarrow\; \eta < 0.774$$

```python
for eta_test in [0.05, 0.5, 0.774, 1.006, 1.935]:
    _, hist = run_gd(p_fit, eta=eta_test, steps=25)
    print(eta_test, [round(v, 4) for v in hist[::5]])
```

```text
0.050  [0.2863, 0.2859, 0.2855, 0.2853, 0.2851, 0.2849]
0.500  [0.2863, 0.2845, 0.2844, 0.2844, 0.2844, 0.2844]
0.774  [0.2863, 0.2844, 0.2844, 0.2844, 0.2844, 0.2844]
1.006  [0.2863, 0.2844, 0.2844, 0.2852, 0.3674, 9.4746]
1.935  [0.2863, 0.2866, 1684.611, 728.149, 728.149, 728.149]
```

Below $2/L$: settles. Just above it ($1.006 = 1.3\times$ the bound): looks fine for three steps,
then visibly comes apart by step 20 — the same delayed-onset instability Part 7.3 showed, not an
instant explosion. Well above it ($1.935 = 2.5\times$): gone by step 10. The bound from Part 7.4
was not a toy-problem curiosity. It is doing real work here, on a real, noisy, multi-feature,
nonlinear fit.

## 9.8 What the closed form in Part 6 would have missed

Solve the normal equations anyway, pretending the relationship is linear:

```python
X_design = np.column_stack([X, np.ones(n)])
beta_ols = np.linalg.solve(X_design.T @ X_design, X_design.T @ y)
print(beta_ols, np.mean((X_design @ beta_ols - y) ** 2))
```

```text
[12.42532432  7.35148276  4.12005609 23.67327072] loss=1.3128288212841295
```

Loss $1.313$, against $0.286$ for the trained softplus model — more than four times worse. Part
6's exact, one-shot solution is genuinely unbeatable *when the model is actually linear*. Here it
is not, and no amount of matrix algebra fixes that; only the nonlinearity you fit with chain rule
and gradient descent does. This is the honest boundary of Part 6's result, not a flaw in it.

## What just happened

Every tool in this document ran once, on one real problem, in sequence:

1. Standardised features (5.1) $\to$ 2. A composed nonlinear model (3.7 + 6.1) $\to$
3. Chain-rule gradient (2.4, 3.7) $\to$ 4. Gradient check (8.2) $\to$ 5. Deliberate breakage (2.6)
$\to$ 6. Hessian and the discovery that this surface is not globally convex (4.2, 4.3) $\to$
7. Gradient descent to a local bowl (7.2) $\to$ 8. The stability bound, tested and holding
(7.4) $\to$ 9. A comparison against the closed form that shows exactly what it cannot do (6.3).

Nothing above required a new mathematical idea. It required noticing that the eight ideas you
already have compose.

## Exercises, Part 9

**9.1 [core]** Rerun 9.1–9.4 with a different `np.random.seed`. Confirm the gradient check still
lands below $10^{-7}$.

**9.2 [core]** Add a fourth feature (invent one — e.g. `distance_to_grid_km`) and rerun the whole
pipeline unmodified except for the new column. What changes, and what doesn't?

**9.3 [proof]** Derive $\partial L/\partial z$ in section 9.3 on paper without looking, for a
generic activation $\phi$ in place of softplus. Confirm your general formula reduces to the boxed
one when $\phi = \text{softplus}$.

**9.4 [stretch]** Replace `run_gd`'s fixed $\eta$ with momentum (Part 7.5, now actually
implemented) and compare the number of steps needed to reach loss $< 0.29$ against plain descent
from the same start point.

**9.5 [stretch]** Extend the `Value` autodiff engine from 8.4 with a `.sigmoid()` and
`.softplus()` method, and rebuild this entire capstone using `Value` objects instead of NumPy
gradients. Confirm the final loss matches, and confirm `E.backward()` produces the same gradient
as `grad_analytic` at the starting point.
...
**Part 9.** Explain why a nonlinear model's loss surface is not guaranteed convex everywhere,
while a linear model's always is. Explain why the $2/L$ bound had to be evaluated near the
minimum rather than at the start. Explain, in one sentence, what the comparison against the
normal equations in 9.8 actually proves.
